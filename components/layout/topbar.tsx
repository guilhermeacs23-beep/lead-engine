    ...
    <div className="relative">
      ...
      <header className="glass-topbar flex h-[52px] items-center gap-2 px-3 sm:px-5"
        ...
        <button onClick={() => setSearchOpen(true)}
          ...
          <span className="hidden rounded px-1.5 py-0.5 text-[10px] text-white/35 sm:block"
            style={{ border: '0.5px solid rgba(255,255,255,0.10)' }}>⌘K</span>
        </button>
        ...
        <div ref={bellRef} className="relative">
          ...
          {bellOpen && (
            <div className="absolute right-0 top-11 z-50 w-80 max-w-[calc(100vw-1rem)] overflow-hidden rounded-xl"
              style={{ background: 'rgba(13,11,32,0.98)', border: '0.5px solid rgba(255,255,255,0.14)', boxShadow: '0 20px 60px rgba(0,0,0,0.7)', backdropFilter: 'blur(24px)' }}>
              <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.08)' }}>
                ...
                <button onClick={() => setBellOpen(false)} className="text-white/30 hover:text-white/60"><X size={14} /></button>
              </div>
              <div className="max-h-72 overflow-y-auto">
                {notifs.length === 0 ? (
                  ...
                  <div key={n.id} className="flex items-start gap-3 px-4 py-3 hover:bg-white/[0.04]"
                    style={{ borderBottom: '0.5px solid rgba(255,255,255,0.05)' }}>
                    <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg"
                      style={{ background: n.tipo === 'novo_lead' ? 'linear-gradient(135deg,#6366f1,#8b5cf6)' : 'rgba(251,191,36,0.12)' }}>
                      {n.tipo === 'novo_lead'
        ...
        <button onClick={toggleBgPanel}
          ...
          <Globe size={12} strokeWidth={1.5} />Fundo
        </button>
