...
const PAGE_TITLES: Record<string, { title: string; action: string }> = {
  ...
  '/leads':       { title: 'Gerador de Leads', action: 'Buscar Leads'        },
  '/pipeline':    { title: 'Pipeline Kanban',  action: '+ Nova Oportunidade' },
  '/relatorios':  { title: 'Relatórios',       action: 'Exportar'            },
  '/mapa':        { title: 'Mapa Logístico',   action: 'Filtrar Região'      },
}
...
export function Topbar() {
  ...
  const page = PAGE_TITLES[pathname] ?? { title: 'Lead+', action: '+ Criar' }
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="mr-1 text-sm font-medium text-white/90">Lead+</span>
        ...
        <div className="mx-2 flex flex-1 items-center gap-2 rounded-lg px-3 py-1.5"
          ...
          <Search size={13} className="text-white/55" strokeWidth={2} />
          <span className="text-xs text-white/50">Buscar leads, empresas, CNPJ...</span>
          <span className="ml-auto rounded px-1.5 py-0.5 text-[10px] text-white/45"
        ...
        <button onClick={toggleBgPanel}
          className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs text-white/75 transition-all hover:bg-white/10 hover:text-white/95"
